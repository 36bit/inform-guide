...
| `not` | `0x0F` | St | — | Bitwise NOT (v3–4; moves to VAR in v5+) |
...
In versions 3 and 4, `not` is a 1OP instruction that stores its result.
From version 5 onwards, it becomes a VAR instruction at opcode `0x38`.
...
Note: opcode `$0F` in the 1OP range is `not` in versions 3 and 4 (with
store), and `call_1n` from version 5 onwards. From version 5, `not`
