...
> in an `!%` header comment) for a Z-code target is a fatal error,
> *even if the value matches the version's natural byte size*
> (e.g., `$DICT_WORD_SIZE=4` for v3 still fails). Omit the setting
> entirely for Z-code projects.
